#ifndef UI_MULTITABLE_HPP
#define UI_MULTITABLE_HPP

#include <SFML/Graphics.hpp>
#include "multielements.hpp"
#include "concrete_h.hpp"
#include "background.hpp"
#include "multitable_h.hpp"
...
namespace UI {
    ...
    template<class BackgroundType> void Table<BackgroundType>::recomputeSizes() {
        ...
        for (auto& column : elements) {
            std::size_t temp = 0u;
            for (ElementType& element : column) temp += element.getGridSpace().y;
    ...
    template<class BackgroundType> void Table<BackgroundType>::replaceElement(const ElementType& replaced, sf::Vector2i place) {
        ...
        if (place.y == -1) place.y = elements.at(0).size() - 1;
